 ...
 * An object with the following properties:
   ...
   * ``tooltip``: a tooltip to apply to this date, via the ``name`` HTML attribute
 ...
 * An object with the following properties:
   ...
   * ``enabled``: same as the Boolean value above
   * ``classes``: same as the String value above
   * ``tooltip``: a tooltip to apply to this year, via the ``name`` HTML attribute
...
The string that will appear on top of the datepicker. If empty the name will be hidden.
