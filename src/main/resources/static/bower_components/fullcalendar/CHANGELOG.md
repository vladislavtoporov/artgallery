...
Bugfixes:
- stale calendar name when navigate away from then back to the a view (#3604)
...
- name computed incorrectly for views with no weekends (#2884)
...
- events with no name rendering as "undefined"
...
- Fix week view column name for en-gb ([PR220])
- Publish to NPM ([2447])
...
- view.name wasn't defined in viewRender callback ([2407])
...
- View object's `name` property mistakenly omitted ([2407])
